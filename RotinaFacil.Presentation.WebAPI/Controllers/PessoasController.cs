...
namespace RotinaFacil.Presentation.WebAPI.Controllers
{
    ...
    public class PessoasController : ControllerBase
    {
        ...
        {
            //TODO: Criar validação de help para telefone
            //TODO: Criar validação de help endereço

            var consultarPessoaPorNome = await _pessoasService.ObterPorNomeAsync(pessoaDTO.Nome);
            if (string.IsNullOrWhiteSpace(consultarPessoaPorNome.Nome))
                return BadRequest($"Pessoa {consultarPessoaPorNome.Nome} já existe na base.");

            var consultarPessoaPorCPF = await _pessoasService.ObterPorCPFAsync(pessoaDTO.CPF);
            if (string.IsNullOrWhiteSpace(consultarPessoaPorNome.Nome))
                return BadRequest($"Pessoa {consultarPessoaPorNome.Nome} com o CPF: {pessoaDTO.CPF} já existe na base.");
